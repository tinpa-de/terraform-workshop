...
### Schritt 4 – Persönliche State-Lock-Tabelle erstellen

#### Warum braucht Terraform Remote State und ein Lock?

Standardmäßig speichert Terraform seine State-Datei (`terraform.tfstate`) auf deinem lokalen Rechner. Das funktioniert für die Einzelnutzung gut, aber in einer gemeinsamen Umgebung könnten mehrere Personen gleichzeitig `tf apply` ausführen und gegenseitig ihren State überschreiben.

Um das zu verhindern, speichern wir die State-Datei in einem zentralen **S3-Bucket** (bereits für diesen Workshop erstellt). Zusätzlich verwenden wir eine **DynamoDB-Tabelle als Lock**: Wenn jemand `tf apply` ausführt, wird der Lock gesetzt, und alle anderen, die gleichzeitig apply ausführen möchten, müssen warten.

Da du den S3-Bucket mit anderen Workshop-Teilnehmern teilst, erstellt jede Person ihre **eigene DynamoDB-Lock-Tabelle**, damit sich die Locks nicht gegenseitig stören.

**Schritte:**

1. Öffne `terraform/initialize-lock-db/default.auto.tfvars` in deinem Texteditor.
2. Ersetze `-NAME` durch deinen Vornamen oder deine Initialen. Verwende **nur Kleinbuchstaben und Bindestriche** — keine Leerzeichen oder Sonderzeichen.
   ```
   dynamodb_table_name = "terraform-state-lock-NAME"
   ```
3. Navigiere in deinem Terminal in das Verzeichnis `initialize-lock-db`:
   ```bash
   cd terraform/initialize-lock-db
   ```
4. Führe die folgenden Befehle aus. Der erste lädt die erforderlichen Provider-Plugins herunter; der zweite erstellt deine DynamoDB-Tabelle.
   ```bash
   tf init
   tf plan
   tf apply
   ```
5. Terraform gibt eine Zusammenfassung aus, was es erstellen möchte, und fragt dann: `Do you want to perform these actions?` — tippe `yes` und drücke Enter.

Du solltest sehen: `Apply complete! Resources: 1 added, 0 changed, 0 destroyed.`

---

### Schritt 5 – Persönliches Backend konfigurieren

Jetzt musst du dem Haupt-Terraform-Projekt mitteilen, *wo* deine State-Datei gespeichert werden soll. Alle speichern ihren State im selben gemeinsamen S3-Bucket, aber jede Person verwendet einen eindeutigen `key` — als Dateipfad innerhalb des Buckets — damit dein State nicht den eines anderen überschreibt.

1. Öffne `terraform/main.tf` in deinem Editor.
2. Suche den `backend "s3"`-Block nahe dem Anfang der Datei, innerhalb des `terraform {}`-Blocks:
   ```hcl
   backend "s3" {
     bucket = "terraform-state-nl-devk"
     key    = "infrastructure-NAME"   # ← diese Zeile ändern
   }
   ```
3. Ersetze `NAME` im `key`-Wert durch deinen Namen — verwende denselben Wert wie in Schritt 4.
   ```hcl
   key = "infrastructure-NAME"
   ```
4. Solange du diese Datei geöffnet hast, füge auch deinen DynamoDB-Tabellennamen hinzu, damit das State-Locking aktiv ist:
   ```hcl
   dynamodb_table = "terraform-state-lock-NAME"   # gleicher Name wie in Schritt 4
   ```
5. Navigiere in das Verzeichnis `terraform/`:
   ```bash
   cd ..
   ```
6. Initialisiere das Backend:
   ```bash
   tf init
   ```

Terraform verbindet sich mit dem S3-Bucket und bestätigt, dass das Backend bereit ist. Du bist jetzt vollständig eingerichtet und kannst mit den Aufgaben beginnen.
